    ...
    for dir in os.scandir(lambda_functions_folders):
        for filename in os.listdir(dir):
            if filename == "requirements.txt":
                ...
                os.system(
                    f"pip3.9 install -r {dir.path}/requirements.txt --platform manylinux2014_x86_64 --target={dir.path} --implementation cp --only-binary=:all: --upgrade"
                )
...
============================="""
    ...
    sed_cmd = f"sed -i 's/nodejs14/nodejs16/g' {install_directory}/cdk.out/{install_parameters['cluster_id']}.template.json"
    os.system(sed_cmd)
