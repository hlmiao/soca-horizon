...
class SOCAInstall(Stack):
    def __init__(self, scope: Construct, construct_id: str ,**kwargs) -> None:
        ...
        # Init SOCA resources
        if user_specified_variables.region == "cn-north-1" or user_specified_variables.region == "cn-northwest-1":
            self.soca_resources = {
            ...
            "ami_id": user_specified_variables.custom_ami if user_specified_variables.custom_ami else install_props.RegionMap.__dict__[user_specified_variables.region].__dict__['x86_64'].__dict__[user_specified_variables.base_os],
        ...
        self.network()  # Cself.scheduler_ami_id()reate Network environment
        ...
        if install_props.Config.analytics.ignore_opensearch:
            pass 
        else:
    ...
    def secretsmanager(self):
        ...
        if install_props.Config.analytics.ignore_opensearch:
            ...
            # ES configuration
            if not user_specified_variables.os_endpoint:
                if not install_props.Config.analytics.skip_opensearch_secrete: 
                    secret["OSDomainEndpoint"] = self.soca_resources[
                        "os_domain"
                    ].domain_endpoint
                else:
                    secret["OSDomainEndpoint"] = ""
            else:
