...
class CreateLinuxDesktop(Resource):
    ...
    def post(self, session_number):
        ...
        try:
            ...
            elif [[ "''' + base_os + '''" == "ubuntu18" || "''' + base_os + '''" == "ubuntu20" || "''' + base_os + '''" == "ubuntu22" ]];
            then
                    apt-get update
                    apt-get install -y python3-pip
                    PIP=$(which pip3)
                    $PIP install awscli -i ''' + config.Config.PIP_CHINA_MIRROR + '''
            ...
            # End: Check if we're using a customized AMI
            fi
            ...
            # Configure Chrony
            if [[ "''' + base_os + '''" == "ubuntu18" || "''' + base_os + '''" == "ubuntu20" || "''' + base_os + '''" == "ubuntu22" ]];
            then
            ...
            if [[ "$region" == "cn-north-1" || "$region" == "cn-northwest-1" ]]; then
                cp /apps/soca/$SOCA_CONFIGURATION/cluster_node_bootstrap/config_china.cfg /root/config.cfg
            else
