...
def get_compute_pricing(ec2_instance_type):
    ...
    if region in ['cn-north-1' ,'cn-northwest-1']:
        _data = json.load(open('/apps/soca/%SOCA_CONFIGURATION/cluster_analytics/pricing_index.json','r'))
        products = _data['products']
        terms = _data['terms']

        allowed_keys = [ec2_instance_type]

        filled_sku = dict(filter(lambda x: \
            'instanceType' in x[1]['attributes'].keys() \
            and x[1]['attributes']['instanceType'] in allowed_keys \
            , products.items()))

        for sku,v in filled_sku.items():
            response = {}
            response['product'] =  v
            response['terms'] = {}
            filled_terms = dict(filter(lambda x: \
                x[0] == sku ,\
                terms['OnDemand'].items()))
            if len(filled_terms) > 0:
                response['terms']['OnDemand'] = filled_terms[sku]

            filled_terms = dict(filter(lambda x: \
                x[0] == sku ,\
                terms['Reserved'].items()))
            if len(filled_terms) > 0:
                response['terms']['Reserved'] = filled_terms[sku]
                        ...
                        for ratecode in v[skus]["priceDimensions"].keys():
                            ...
                            if (
                                ...
                                pricing["ondemand"] = float(
                                    instance_data["pricePerUnit"]["CNY"]
                                )
                else:
                    for skus in v.keys():
                        if (
