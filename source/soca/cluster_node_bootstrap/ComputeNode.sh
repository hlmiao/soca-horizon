...
if [[ $SOCA_BASE_OS == "ubuntu18" || $SOCA_BASE_OS == "ubuntu20" || $SOCA_BASE_OS == "ubuntu22" ]]; then
  if ! snap services amazon-ssm-agentx; then
     echo "Beging install ssm agent in ubuntu"
     snap install amazon-ssm-agent --classic
     snap start amazon-ssm-agent
  else
     echo "Already installed SSM"
  fi
else
  machine=$(uname -m)
  if ! systemctl status amazon-ssm-agent; then
...
if [[ ! -f /root/soca_preinstalled_packages.log ]]; then
    ...
    if [[ $SOCA_BASE_OS == "rhel7" ]]; then
      ...
      apt-get update
      apt-get install -y $(echo ${SYSTEM_PKGS_UBUNTU22[*]})
    else
      # AL2
    ...
    if [[ $SOCA_BASE_OS == "ubuntu18" ]]; then
      ...
      apt-get update
      DEBIAN_FRONTEND=noninteractive apt-get install -y $(echo ${OPENLDAP_SERVER_PKGS_UBUNTU22[*]} ${SSSD_PKGS_UBUNTU22[*]})
      apt-get install -y adcli
    else
