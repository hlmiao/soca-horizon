...
then
  ...
  if [[ $SOCA_BASE_OS == "ubuntu18" || $SOCA_BASE_OS == "ubuntu20" || $SOCA_BASE_OS == "ubuntu22" ]]; then
        ...
        cat << EOF > /etc/systemd/system/soca-dcv-session.service
[Unit]
Description=/etc/soca-dcv-session
ConditionPathExists=/etc/soca-dcv-session

[Service]
Type=forking
ExecStart=/etc/soca-dcv-session start
TimeoutSec=0
StandardOutput=tty
RemainAfterExit=yes
...
EOF
    ...
    chmod +x /etc/soca-dcv-session
    systemctl enable soca-dcv-session
    systemctl start soca-dcv-session.service
