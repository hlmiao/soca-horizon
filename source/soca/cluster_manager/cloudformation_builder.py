...
# ubuntu
if [[ "''' + params['BaseOS'] + '''" == "ubuntu18" || "''' + params['BaseOS'] + '''" == "ubuntu20" || "''' + params['BaseOS'] + '''" == "ubuntu22" ]];
then
    apt-get update
...
if [[ "$FS_DATA_PROVIDER" == "fsx_lustre" ]] || [[ "$FS_APPS_PROVIDER" == "fsx_lustre" ]]; then
    if [[ -z "$(rpm -qa lustre-client)" ]]; then
        ...
        #ubuntu
           ...
           sudo bash -c 'echo "deb https://fsx-lustre-client-repo.s3.amazonaws.com/ubuntu focal main" > /etc/apt/sources.list.d/fsxlustreclientrepo.list && apt-get update'i
